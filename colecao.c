    ...
    switch(c->estrutura_id){
        ...
        case 1:
            if(c->inicio == NULL){
                ...
                else if(caminho->esq == NULL){
                    //Inserção no início para uma lista com mais de um elemento
                    NO->dir = caminho;
                    NO->esq = NULL;
                    caminho->esq = NO;
                    c->inicio = NO;
                }
                else if(caminho->dir == NULL){
                    //Inserção no fim para uma lista com mais de um elemento
                    NO->esq = caminho;
                    NO->dir = NULL;
                    caminho->dir = NO;
                }
                else{
                    //Inserção no  meio da lista (o novo nó será inserido à esquerda do nó caminho)
                    NO->esq = caminho->esq;
                    NO->esq->dir = NO;
                    NO->dir = caminho;
                    caminho->esq = NO;
...
int existe(Colecao* c, int valor)
{
    ...
    switch(c->estrutura_id){
        case 1:
            ...
            while(caminha != NULL && valor >= caminha->valor){
    ...
    switch(c->estrutura_id){
        ...
        case 1:
            if(c->inicio == NULL){
                ...
                No *caminho = c->inicio;
                while((valor > caminho->valor) && (caminho->dir != NULL))
                    //Enquanto o próximo elemento for maior e não for o fim da lista
                    caminho = caminho->dir;
                if(caminho->esq == NULL && caminho->dir == NULL){
                    //A lista possui apenas um nó, e o laço anterior não nos mostra onde devemos inserir o novo nó
